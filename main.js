...
var $author$project$Main$gotItem = _Platform_incomingPort(
	'gotItem',
	A2(
			...
			switch (instr.$) {
				case 'Increment':
					...
					var updatedModel = _Utils_update(
						model,
						{
							highlighted: A3($elm$core$Dict$insert, reg, 'bg-green-200', model.highlighted),
				...
				case 'Decrement':
					...
					var updatedRegisters = A3(
						...
						$elm$core$Maybe$map(
							function (val) {
								return A2($elm$core$Basics$max, 0, val - 1);
							}),
						model.registers);
					var updatedModel = _Utils_update(
						model,
						{
							highlighted: A3($elm$core$Dict$insert, reg, 'bg-red-200', model.highlighted),
							instructionPointer: nextInstructionPointer,
							registers: updatedRegisters
						});
					return _Utils_Tuple2(
...
var $author$project$Main$setItem = _Platform_outgoingPort(
	'setItem',
	function ($) {
		var a = $.a;
		var b = $.b;
					...
					function (index, instruction) {
						...
						var instructionText = function () {
							switch (instruction.$) {
								case 'Increment':
									var n = instruction.a;
									return 'Add ' + $elm$core$String$fromInt(n);
								case 'Decrement':
									var n = instruction.a;
									return 'Sub ' + $elm$core$String$fromInt(n);
								case 'StartLoop':
									return '(';
								case 'EndLoop':
						...
						return A2(
							...
							_List_fromArray(
								[
									...
									_Utils_ap(
										baseClasses,
										_Utils_ap(typeColorClasses, activeClasses)))
								]),
							_List_fromArray(
								[
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$Attributes$class('flex gap-4 w-1/3 pr-3')
							]),
						_List_fromArray(
							[
								function () {
								if (model.isRunning) {
									...
									var isDisabled = atEndOfInstructions;
									return A2(
										$elm$html$Html$button,
										_Utils_ap(
											_List_fromArray(
											...
											isDisabled ? _List_fromArray(
												[
													...
													$elm$html$Html$Attributes$class('bg-gray-400 cursor-not-allowed')
												]) : _List_Nil),
										_List_fromArray(
											[
								...
								function () {
								var isDisabled = atEndOfInstructions || model.isRunning;
								return A2(
									$elm$html$Html$button,
									_Utils_ap(
										_List_fromArray(
								...
								_List_fromArray(
									[
										$elm$html$Html$Attributes$class('w-1/3 px-4 py-2 bg-gray-400 text-white flex cursor-not-allowed items-center justify-center rounded'),
										$elm$html$Html$Attributes$disabled(true)
									]),
								_List_fromArray(
									[
						...
						_List_fromArray(
							[
								$elm$html$Html$Attributes$class('flex gap-4 w-1/3 pr-8')
							]),
						_List_fromArray(
							[
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('flex flex-grow gap-4 overflow-hidden')
					]),
				_List_fromArray(
					[
						A2(
